...
def ProductCrawl(index, url_product):
    sleep(sleep_random())
    ...
    SKU_code = str(
        lxmlElem1.xpath('//div[@class="dy"]/*//div[@class="u-hm"]/form[1]/input[@name="item[product_sku_id]"]/@value'))
    SKU_code = clean_rubish(SKU_code)
    SKU_code = SKU_code.replace("'", "")
    SKU_code = SKU_code.replace('"', "")
    product_id = str(lxmlElem1.xpath(
        '//div[@class="c-panel__body"]/*//div[@class="u-clearfix"]/form[1]/input[@name="item[product_id]"]/@value'))
    ...
    Listing_size = str(
        lxmlElem1.xpath('//*[@id="section-informasi-barang"]/div[2]/div[2]/div/div[1]/table/tbody/tr[8]/td[2]/text()'))
    if Listing_size == '':
        Listing_size = 'NA'
    Listing_size_data_ = clean_rubish(
        str(lxmlElem1.xpath('//option[@data-label="Kapasitas" and @selected ]/@data-id')).replace("'", ""))
    Listing_color_id = clean_rubish(
        str(lxmlElem1.xpath('//tr[td[contains(text(),"Warna")]]/td[2]/select/option[@selected]/@data-id')).replace("'",
                                                                                                                   ""))
    ...
    try:
        listing_price = str(
            lxmlElem1.xpath('//*[@id="section-main-product"]/div[2]/div[2]/div/div[1]/div[1]/span/text()'))
    ...
    if promo_price == '[]' or promo_price == '':
        try:
            ...
            promo_price = lxmlElem1.xpath(
                '//div[@class="c-product-detail-price"]/span/span[@class="amount positive"]/text()')
    ...
    refurbish = ''
    if any(val.lower() in product_name.lower() for val in
           ['referbish', 'refer', 'refurbish', 'refurb', 'second hand', 'second-hand', 'second', '2 nd', '2nd', 'bekas',
            'welfare goods', 'welfare products']):
        refurbish = 'Yes'
    else:
        refurbish = 'No'
    ...
    seller_rating = str(
        lxmlElem1.xpath('//*[@id="section-informasi-pelapak"]/div[2]/div/div[2]/div[1]/div[1]/div/span/text()'))
    if seller_rating == '':
        seller_rating = 'NA'

    product_rating = str(
        lxmlElem1.xpath('//*[@id="section-ulasan-barang"]/div[2]/div/div[1]/div[1]/div[1]/div[1]/div/div/span/text()'))
    if product_rating == '':
        product_rating = 'NA'
    ...
    warranty = lxmlElem1.xpath(
        '//*[@id="section-informasi-barang"]/div[2]/div[2]/div/div[1]/table/tbody/tr[10]/td[2]/text()')
...
ProductCrawl(0,
             'https://www.bukalapak.com/p/handphone/hp-smartphone/fhho1l-jual-promo-samsung-galaxy-s8-sm-g950-garansi-resmi-samsung')
